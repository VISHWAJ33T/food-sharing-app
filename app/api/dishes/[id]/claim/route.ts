...
export async function POST(
  ...
  try {
    ...
    if (dish.claimed) {
      return NextResponse.json({ error: 'Dish already claimed' }, { status: 400 })
    }

    const existingRequest = dish.claimRequests.find(
      (request) => request.userId === session.user.id && request.status === 'PENDING'
    )

    if (existingRequest) {
      return NextResponse.json({ error: 'You have already requested this dish' }, { status: 400 })
    }
