...
export default function ManageClaim({ params }: { params: { id: string } }) {
  ...
  const handleClaimAction = async (requestId: string, status: 'APPROVED' | 'REJECTED') => {
    try {
      ...
      toast({ title: `Failed to ${status.toLowerCase()} claim request`, variant: 'destructive' })
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-100 to-green-200 p-4">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 className="text-3xl font-bold text-green-800 mb-6">Manage Claims for {dish.name}</h1>
          ...
          <div className="space-y-4">
            {dish.claimRequests.map((request) => (
              <div key={request.id} className="border rounded-lg p-4">
                <p className="text-gray-600 mb-2">Requested by: {request.user.name}</p>
                <p className="text-gray-600 mb-4">Status: {request.status}</p>
                {request.status === 'PENDING' && (
                  <div className="flex space-x-2">
                    <Button onClick={() => handleClaimAction(request.id, 'APPROVED')} className="bg-green-500 hover:bg-green-600">
                      Approve
                    </Button>
                    <Button onClick={() => handleClaimAction(request.id, 'REJECTED')} className="bg-red-500 hover:bg-red-600">
                      Reject
                    </Button>
                  </div>
                )}
        ...
        <Button onClick={() => router.push(`/dish/${params.id}`)} className="w-full mt-6">
